...
export const fetchWithToken = (url, method = 'get', data = {}) => (dispatch, getState) => {
  ...
  return axios({
    ...
    .catch(r => {
      ...
      if (refreshToken && r.response && r.response.statusText === 'Unauthorized') {
        return dispatch(useRefreshToken(refreshToken))
          .catch(() => {
            dispatch(logOutActionCreator())
            dispatch(addSnackbarActionCreator('Twoja sesja wygasła. Zaloguj się ponownie.', 'red'))
            return Promise.reject(r)
          })
          .then(() => axios({
            url: getUrlWithToken(),
...
export const changePasswordAsyncActionCreator = (oldPassword, newPassword) => (dispatch, getState) => {
  ...
  return axios({
    ...
    .then(r => {
      ...
      dispatch(addSnackbarActionCreator('Hasło zostało zmienione.'))
      return r
    })
    .catch((r) => {
...
export const checkIsUserLoggedIn = () => (dispatch) => {
  ...
  const expires = false && idToken ? jwt.decode(idToken).exp * 1000 : 0
